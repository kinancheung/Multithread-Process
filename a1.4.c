...
static pthread_mutex_t mutexPool;
static struct block *blockState;
static int freeThreads = 0;
static sem_t *semState;
...
void *thread_loop(void *args){
  ...
  while(1){
    ...
    pthread_mutex_lock(&mutexPool);
    freeThreads++;
    pthread_cond_wait(&condPool, &mutexPool);
    freeThreads--;
...
int main(int argc, char *argv[]) {
    ...
    // Make pthread pool here
    pthread_t threadPool[NUM_THREADS];
    pthread_mutex_init(&mutexPool, NULL);
    pthread_cond_init(&condPool, NULL);
    sem_t sem;
    sem_init(&sem, 0, 0);
    semState = &sem;

    for (int i = 0; i < NUM_THREADS; i++){
      if(pthread_create(&threadPool[i], NULL, &thread_loop, NULL) !=0) {
